...
class TypeNamer {
	...
	public static function primitives(s:String):String {
		var m = [
			"Bool" => "bool",
			"Void" => "",
			"Int" => "int",
		...
		try {
			return switch type {
						...
						return switch (t.get()) {
							...
							case {pack: pack, name: name}: {
									...
									var generics = if (generic_params != null && generic_params.length > 0) {
										...
										t.get().params.length > 0 ? '[/*gen */' + params.map(type_param_to_string).join(",") + "]" : "";
				...
				case TAbstract(ta, params): switch ta.get() {
						...
						case {name: something}: {
                                var try_prim=primitives(something);
                                if (try_prim!=null) return try_prim;
...
function type_param_to_string(tin:haxe.macro.Type) {
	return switch tin {
		case TInst(tinn, params):
            ...
            var try_prim=TypeNamer.primitives(name);
            if (try_prim!=null) return try_prim;
