...
class StringTools:
    ...
    __slots__ = ()
    _hx_statics = ["isSpace", "ltrim", "rtrim", "trim", "rpad"]
...
class sys_FileSystem:
    ...
    __slots__ = ()
    _hx_statics = ["exists", "readDirectory"]
...
class TestRunner:
    _hx_class_name = "TestRunner"
    __slots__ = ()
    _hx_statics = ["main", "runTest", "printResults"]

    @staticmethod
    def main():
        args = Sys.args()
        folderPath = (args[0] if 0 < len(args) else None)
        if (folderPath is None):
            _hx_str = "Please provide the folder path as an argument."
            python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
            return
        results = list()
        test = (args[1] if 1 < len(args) else None)
        if (test is not None):
            t = TestRunner.runTest(test,test,(((("null" if folderPath is None else folderPath) + "/") + ("null" if test is None else test)) + ".txt"))
            results.append(t)
            TestRunner.printResults(results)
            return
        files = sys_FileSystem.readDirectory(folderPath)
        _g = 0
        while (_g < len(files)):
            file = (files[_g] if _g >= 0 and _g < len(files) else None)
            _g = (_g + 1)
            if file.endswith(".hx"):
                baseName = HxString.substr(file,0,(len(file) - 3))
                print(str(baseName))
                hxFile = ((("null" if folderPath is None else folderPath) + "/") + ("null" if file is None else file))
                txtFile = (((("null" if folderPath is None else folderPath) + "/") + ("null" if baseName is None else baseName)) + ".txt")
                if sys_FileSystem.exists(txtFile):
                    print("Running...")
                    result = TestRunner.runTest(baseName,hxFile,txtFile)
                    results.append(result)
        TestRunner.printResults(results)

    @staticmethod
    def runTest(baseName,hxFile,txtFile):
        cmd = "C:/HaxeToolkit/haxe/haxe.exe"
        args = ["-main", ("tests." + ("null" if baseName is None else baseName)), "-D", "go_output=haxe_out", "-L", "reflaxe_go_inter"]
        compileProcess = sys_io_Process(cmd,args)
        compileProcess.stdout.readAll()
        compileResult = compileProcess.exitCode()
        if ((compileResult != 0) and ((compileResult != 2))):
            return TestResult(baseName,False,(((((("Compilation failed " + Std.string(compileResult)) + " ") + Std.string(compileProcess.stderr.readAll())) + ("null" if cmd is None else cmd)) + " ") + HxOverrides.stringOrNull(" ".join([python_Boot.toString1(x1,'') for x1 in args]))))
        print("Compilation succeeded")
        runProcess = sys_io_Process("go",["run", "main.go"])
        output = runProcess.stdout.readAll().toString()
        if (output == ""):
            return TestResult(baseName,False,"go output was empty, likely some error is GO111MODULE off?")
        runProcess.close()
        expectedOutput = sys_io_File.getContent(txtFile)
        success = (StringTools.trim(output) == StringTools.trim(expectedOutput))
        message = ("Passed" if success else ((("Fail: Output doesn't match," + ("null" if output is None else output)) + ",") + ("null" if expectedOutput is None else expectedOutput)))
        return TestResult(baseName,success,message)

    @staticmethod
    def printResults(results):
        _hx_str = "Test Results:"
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        _hx_str = "-------------------------------------------------------------------"
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        _hx_str = "| File Name | Status | Message                                    |"
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        _hx_str = "-------------------------------------------------------------------"
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        _g = 0
        while (_g < len(results)):
            result = (results[_g] if _g >= 0 and _g < len(results) else None)
            _g = (_g + 1)
            status = ("Pass" if (result.success) else "Fail")
            _hx_str = Std.string((((((("| " + HxOverrides.stringOrNull(StringTools.rpad(result.fileName," ",9))) + " | ") + HxOverrides.stringOrNull(StringTools.rpad(status," ",6))) + " | ") + HxOverrides.stringOrNull(StringTools.rpad(result.message," ",45))) + " |"))
            python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
        _hx_str = "-------------------------------------------------------------------"
        python_Lib.printString((("" + ("null" if _hx_str is None else _hx_str)) + HxOverrides.stringOrNull(python_Lib.lineEnd)))
...
class TestResult:
    _hx_class_name = "TestResult"
    __slots__ = ("fileName", "success", "message")
    _hx_fields = ["fileName", "success", "message"]

    def __init__(self,fileName,success,message):
